...
# --------------------------------------------------------------------------------------
# Requisitos:
#   pip install streamlit chronos-forecasting plotly scikit-learn pandas numpy torch
# Execução:
...
Usamos um subconjunto do **Australian Electricity**:  
`item_id, timestamp, target` → agregados **mensalmente** por `item_id`. Você escolhe o `item_id` no app.

**Fonte:** https://autogluon.s3.amazonaws.com/datasets/timeseries/australian_electricity_subset/test.csv
...
st.caption("Autor: **Dr. Silvio da Rosa Paula** · GitHub: https://github.com/silviopaula · LinkedIn: https://www.linkedin.com/in/silvio-paula/")
...
with tab_app:
    ...
    with st.sidebar:
        ...
        demo_url = "https://autogluon.s3.amazonaws.com/datasets/timeseries/australian_electricity_subset/test.csv"
        ...
        local_base_dir = None
        if force_offline:
            # Sem caminho pré-carregado — o usuário informa se desejar
        ...
        q_mode = st.radio("Escolha", options=["Usar interval_alpha", "Listar quantile_levels"], index=0, horizontal=False)
    ...
    raw_demo = None
    if up is None:
        st.info("Nenhum arquivo enviado — carregando **dataset demo** (Australian Electricity).")
        try:
            raw_demo = pd.read_csv(demo_url)  # item_id, timestamp, target
            raw_demo["timestamp"] = pd.to_datetime(raw_demo["timestamp"], errors="coerce")

            # escolher série na sidebar
            with st.sidebar:
                st.subheader("Demo: seleção de série")
                item_demo = st.selectbox("item_id (demo)", sorted(raw_demo["item_id"].unique()))

            df_item = raw_demo.loc[raw_demo["item_id"] == item_demo].copy()
            # agrega para mensal (soma)
            df_monthly = (
                df_item.set_index("timestamp")["target"]
                .resample("MS").sum()
                .reset_index()
            )
            # Mantém nomes originais — usuário escolherá as colunas adiante
            df = df_monthly.rename(columns={"timestamp": "timestamp", "target": "target"})
            st.success(f"Dataset demo carregado (item_id={item_demo}) — {df.shape[0]} linhas")
        except Exception as e:
    ...
    # Sugestões de default (heurística simples)
    default_date = next((c for c in all_cols if c.lower() in ["data", "date", "timestamp", "time"]), all_cols[0])
    default_target = next((c for c in all_cols if c.lower() in ["target", "valor", "y", "consumo", "ec_total_br"]), all_cols[-1])
